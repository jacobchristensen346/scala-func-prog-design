...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction { (arr: Json) =>
      arr match
        case x: Json.Arr =>
          val listResults = x.items.map(decoder.decode)
          if listResults.forall(_.isDefined) then Some(listResults.flatten)
          else None
        case other =>
          None
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person]((p: (String, Int)) => Person(p._1, p._2))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people")
      .transform[Contacts](clist => clist.people)

  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts]((plist: List[Person]) => Contacts(plist))
