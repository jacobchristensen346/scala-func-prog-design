...
class BloxorzSuite extends munit.FunSuite:
  ...
  test("find neighbors with history") {
    new Level1:
      ...
      val implNeigh = neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Move.Left,Move.Up)).toSet
      assertEquals(testNeigh, implNeigh)
  }

  test("find only new neighbors") {
    new Level1:
      val testNewNeigh = Set(
        (Block(Pos(2,1),Pos(3,1)), List(Move.Down,Move.Left,Move.Up))
      ).to(LazyList)
      val implNewNeigh = newNeighborsOnly(
        Set(
          (Block(Pos(1,2),Pos(1,3)), List(Move.Right,Move.Left,Move.Up)),
          (Block(Pos(2,1),Pos(3,1)), List(Move.Down,Move.Left,Move.Up))
        ).to(LazyList),
      ...
      assertEquals(testNewNeigh, implNewNeigh)
  }
